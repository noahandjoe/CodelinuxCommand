...
void show_info(struct utmp* utbufp){
    char* UTTYPE[9] = {"","","","","","INIT_PROCESS","LOGIN_PROCESS","USER_PROCESS","DEAD_PROCESS"};
    // -是左对齐，8 是指定输出字符串占用的宽度，
    //.8 是指截取字符串的长度为8，
    //最后的 s 是字符串类型。
    printf("%-8.8s ", utbufp->ut_user);
    printf("%-6s ", utbufp->ut_line);
    printf("%-14s ",UTTYPE[utbufp->ut_type]);
    printf("pid: %-10d ",utbufp->ut_pid);
    printf("exit: %6d ",utbufp->ut_exit.e_termination);
